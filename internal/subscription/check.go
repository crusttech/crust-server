...
func Check(ctx context.Context) (err error) {
	...
	for {
		...
		if try >= checkMaxTries {
			return errors.Wrap(err, "unable to check for subscription, reached max tries")
		}
	...
	if !p.IsValid() {
		return err
	} else if p.Domain != configuredDomain {
		return errors.Errorf("subscription domains do not match (%s <> %s)", p.Domain, configuredDomain)
	}
